...
**Not built:** a second *live-recorded* capability, and the attempt is more interesting than the omission. The read flow was recorded by `gpt-4o` first time, and its transcript is what `--mock` replays. The write flow — twelve steps with two human authorisation gates — I tried live three times: one run completed but compiled the account nickname as a *literal*, giving a capability that could only ever open one account name; the other two stopped at the review screen, and the compiler refused to write anything because the recording declared no success condition and no outputs. So the shipped write artifact comes from a deterministic plan.

That one-in-three is worth stating rather than hiding, because it is the argument for the whole architecture. If discovery were reliable you could run the model every time and skip artifacts entirely. It is not — so the model's output is compiled into something a human reviews and a deterministic engine replays, and the guard that refused two unusable recordings is the same one that stops a bad recording reaching production. `scripts/record_flow_b.py --live` reproduces the attempt.

Also not built: an **MCP transport** in front of the catalogue — `cua catalog` emits the tool definitions, so the contract exists and only the wire is missing. **Desktop and terminal surfaces.** **Bounded LLM recovery** on replay failure. **Queues, workers, Docker**, which the brief says are not rewarded. **Operator identity** — the console has no login, which is fine locally and unacceptable in production.
